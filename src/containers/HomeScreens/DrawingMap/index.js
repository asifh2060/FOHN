...
export default class DrawingMap extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      value3: true,
      nearByLocations:
        'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=32.162369,74.183083&radius=1000&type=gas_station&key=AIzaSyDD4wosTgxAGxjTKfG1kN_QJTqStTudWG8',
      markers: [
        {
          title: 'Gujranwala',
          coordinates: {
  ...
  ApplyRadius = () => {
    if (
      ...
      this.state.editing.coordinates.length == 0
    ) {
      Alert.alert(
        null,
        'Please select area',
    ...
    if (params) {
      ...
      if (radius && typeof radius == 'function') {
        ...
        radius([...this.state.polylines, this.state.editing]);
      }
      if (updateIsFromDraw && typeof updateIsFromDraw == 'function') {
        // radius(this.state.radius, this.state.lat, this.state.long);
        updateIsFromDraw([...this.state.polylines, this.state.editing]);
      }
          ...
          <View style={style.mainContainer}>
            <View
              ...
              <TouchableOpacity
                ...
                <Text
                  style={{ fontSize: 22, color: '#00B7B0', fontWeight: 'bold' }}>
                  Cancel
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                <Text
                  style={{ fontSize: 22, color: '#707071', fontWeight: 'bold' }}>
                  Apply
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <MapView
              ...
              //     // coordinated = this.state.porrlylines;
              //     coordinated.push({
              //         // name: 'new1',
            ...
            <TouchableOpacity
              onPress={() => this.setState({ polylines: [], editing: null })}
              style={{
                height: 60,
